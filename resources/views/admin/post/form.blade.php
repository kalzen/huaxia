...
@section('content')
    @if (isset($record))
        ...
        @elseif ($lang == null)
            ...
            @else
                <form method="POST" action="{{ route('admin.post.store-lang', [$post, $lang]) }}">
    @endif
    @csrf
    <div class="content">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control maxlength" maxlength="255" required id="title"
                                name="title" value="{{ old('title') ?: $record->title ?? '' }}">
                            <span class="text-muted">Most optimal 10 - 70 characters.</span>
                            @error('title')
                                <label id="title-error" class="validation-invalid-label"
                        ...
                        <div class="form-group">
                            ...
                            @elseif (isset($record) && $record->images)
                                <input type="hidden" name="image" value="{{ $record->images->first()->url ?? '' }}"
                                    id="image">
                                <img class="mt-2" id="image_preview" height="100"
                                    src="{{ $record->images->first()->url ?? '' }}" />
                            @else
                                <input type="hidden" name="image"
                                    value="{{ asset('hocvienielts/images/no-image.jpg') }}" id="image">
                                <img class="mt-2" id="image_preview" style="display:none;" height="100" />
                            @endif
                            @error('image')
                                <label id="image-error" class="validation-invalid-label"
                                    for="image">{{ $message }}</label>
                            @enderror
                        </div>
                        <div class="form-group">
                            <label class="font-weight-semibold">Từ khoá chính <span class="required"></span></label>
                            <input type="text" class="form-control maxlength" maxlength="255" required id="keyword"
                                name="keyword" value="{{ old('keyword') ?: $record->keyword ?? '' }}">
                        </div>

                        <div class="form-group">
                            <label class="font-weight-semibold">Thẻ</label>
                            @if (old('tags'))
                                <input type="text" class="form-control tokenfield" value="{{ old('tags') }}"
                                    name="tags" data-fouc>
                            @elseif(isset($record) && $record->tags)
                                <input type="text" class="form-control tokenfield"
                                    value="{{ $record->tags->pluck('name')->join(', ') }}" name="tags" data-fouc>
                            @else
                                <input type="text" class="form-control tokenfield" value="" name="tags"
                                    data-fouc>
                            @endif
                        </div>
                        ...
                        <div id="SEO"> <b>PHÂN TÍCH SEO:</b>
                            <div id="issue_seo"> Các vấn đề cần khác phục:
                                <ul>
                                    <li class="issue_title"><b>Tiêu đề chứa từ khoá:</b> Cần sửa tiêu đề có chứa từ khoá
                                    </li>
                                    <li class="issue_description"><b>Mô tả chứa từ khoá:</b> Cần sửa mô tả ngắn có chứa từ
                                        khoá</li>
                                    {!! $issue ?? '' !!}
                    ...
                    <div class="card">
                        <div class="card-body text-center">
                            <div class="form-check form-check-inline disabled">
                                <label class="form-check-label">
                                    <input type="radio" class="form-input-styled"
                                        {{ isset($record) && $record->status == 1 ? 'checked' : '' }} name="status"
                            ...
                            <div class="form-check form-check-inline disabled">
                                <label class="form-check-label">
                                    <input type="radio" class="form-input-styled"
                                        {{ isset($record) && $record->status == 0 ? 'checked' : '' }} name="status"
                                        value="0" data-fouc>
                                    Ẩn
                                </label>
                            </div>
                    ...
                    <div class="card">
                        <div class="card-body text-center">
                            <div class="form-check form-check-inline disabled">
                                <label class="form-check-label">
                                    <input type="radio" class="form-input-styled"
                                        {{ isset($record) && $record->is_promotion == 1 ? 'checked' : '' }}
                                        name="is_promotion" value="1" checked data-fouc>
                                    Tin khuyến mại
                                </label>
                            </div>
                            ...
                            <div class="form-check form-check-inline disabled">
                                <label class="form-check-label">
                                    <input type="radio" class="form-input-styled"
                                        {{ isset($record) && $record->is_promotion == 0 ? 'checked' : '' }}
                                        name="is_promotion" value="0" data-fouc>
                                    Tin tức
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div class="card-body" id="select_category">
                            @include('admin.shared.select-category', [
                                'selected' => isset($record) ? $record->categories->pluck('id')->all() : null,
                            ])
                            @error('category_id')
